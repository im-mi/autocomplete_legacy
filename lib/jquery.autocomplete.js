...
(function($) {
    ...
    $.Autocompleter = function($elem, options) {
        ...
        /**
         * Recalculate the autocomplete window's position when the document scrolls.
         *
         * This keeps the autocomplete window at the correct location if the input field
         * is fixed in place, at least in the common case of it being document-relative.
         *
         * This should only be seen as a temporary solution because it might not work well
         * with some rendering engines (ex. those that use asynchronouse panning).
         * (See https://developer.mozilla.org/docs/Mozilla/Performance/ScrollLinkedEffects ).
         */
        $(document).scroll(function() { self.position(); });
    ...
    $.Autocompleter.prototype.cacheRead = function(filter) {
        return this.cacheData_[String(filter)];
    };
    ...
    $.Autocompleter.prototype.focusItem = function(item) {
        ...
        if ($items.length) {
            ...
            if ($item) {
                ...
                var $acResults = $items.parent();

                var scrollTop = $items.scrollTop();
                var itemTop = $item.offset().top - $items.offset().top + scrollTop;
                ...
                if (itemTop < scrollTop) {
                    ...
                    var scrollBottom = scrollTop + itemsHeight;
                    var itemBottom = itemTop + itemHeight;
                    if (itemBottom >= scrollBottom) {
                        scrollTop = itemBottom - itemsHeight;
                    }
                }
