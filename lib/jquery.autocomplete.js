    ...
    $.Autocompleter.prototype.focusItem = function(item) {
        ...
        if ($items.length) {
            ...
            if ($item) {
                ...
                var acHeight = $acResults.innerHeight();
                var itemHeight = $item.outerHeight();
                var itemTop = $item.offset().top - $items.offset().top;
                var scrollTop = $acResults.scrollTop();

                if (itemTop < scrollTop) {
                    scrollTop = itemTop;
                } else {
                    var scrollBottom = scrollTop + acHeight;
                    var itemBottom = itemTop + itemHeight;
                    if (itemBottom >= scrollBottom) {
                        scrollTop = itemBottom - acHeight;
                    }
                }

                $acResults.scrollTop(scrollTop);
            }
        }
    };
